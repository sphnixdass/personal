...
# data_dir = Path("./img/")
data_dir2 = Path("X:\\Coding\\Python\\ChequeRead\\Pred\\")
...
characters = sorted(list(['0', '1', '2', '3', '4']))
...
model = keras.models.load_model("S:\\InnovationDevelopment\\PythonImgReader\\HeaderImg\\DassHeader")
...
for batch in validation_dataset.take(1):
    ...
    pred_texts, logs = decode_batch_predictions(preds)
    ...
    for x in range(0, len(pred_texts)):
        ...
        resulttemp.append(100 - (logs[x].numpy() * 100))
...
temp_y = []
for x in resultcal:
    temp_y.append(int((x[1].split("\\")[-1]).split("_")[2]))
    print((x[1].split("\\")[-1]).split("_")[2])
